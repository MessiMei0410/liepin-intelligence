...
class AgentDbCase(unittest.TestCase):
    def setUp(self) -> None:
        ...
        # S4-1：该画像补 target_companies_json，使四锚点仅缺“客户的客户”一项（缺失<2），
        # 既有 Copilot 寻访用例继续走“直接建工作流”路径；缺锚 ≥2 的提问清单路径由
        # tests/test_strategy_v2_s4.py 的稀疏 JD 岗位覆盖（PRD §1 口径）。
        conn.execute(
            "INSERT INTO position_profiles VALUES (60,'长越科技','机械高级工程师','本科','7年以上','[\"7年以上精密设备机械设计经验\"]','[\"有限元\"]','[\"ASM\",\"应用材料\"]','[]','[]','[10]','2026-07-14','[\"半导体设备经验\"]','[]','[]','精密设备机械核心岗')"
        )
        raw = {
