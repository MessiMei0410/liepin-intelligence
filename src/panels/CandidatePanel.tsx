...
import { Ban, BriefcaseBusiness, Check, ChevronLeft, CircleCheck, ClipboardCheck, Clock3, ExternalLink, LoaderCircle, MessageSquareText, Search, ShieldCheck, TriangleAlert, UserRoundSearch, X } from 'lucide-react'
...
export function CandidatePanel({value,close,changed}:{value:CandidateDetail,close:()=>void,changed:()=>void|Promise<void>}) {
  ...
  const commitAction=async()=>{ if(!pendingAction)return; const {action,token}=pendingAction; setBusy(`commit:${action}`); setFeedback(undefined); try { if(action==='stop'){ await api.commit(value.id, action, token, actionNote.trim(), actionReason) } else { await api.commit(value.id,action,token,actionNote.trim()) } setPendingAction(undefined); await changed(); setFeedback({tone:'success',text:action==='review'?'评分复核结论已记录到候选人事件。':`${candidateActionLabels[action]}已完成，候选人状态已更新。`}) } catch(e) { setFeedback({tone:'error',text:copilotText(e)||'操作提交失败，请重试。'}) } finally { setBusy('') } }
