...
import { Dashboard, DashboardCounts, DashboardWorkflow, Job, Candidate, Workbench, WorkbenchItem, AnalysisTemplate } from '../api'
...
export function Overview({ dashboard, jobs, candidates, workbench, templates = [], openWorkflow, openCandidate, openAnalysis, archiveWorkflow, openCopilot, quickAnalysis, runTemplate, openTemplate, createTemplate, manageTemplate, analysisBusy }: { dashboard?: Dashboard; jobs: Job[]; candidates: Candidate[]; workbench?: Workbench; templates?: AnalysisTemplate[]; openWorkflow: (id: string) => void | Promise<void>; openCandidate: (id: number) => void | Promise<void>; openAnalysis?: (id: string) => void | Promise<void>; archiveWorkflow: (id: string) => void | Promise<void>; openCopilot?: () => void; quickAnalysis?: () => void; runTemplate?: (id: string) => void; openTemplate?: (template: AnalysisTemplate) => void; createTemplate?: () => void; manageTemplate?: (template: AnalysisTemplate) => void; analysisBusy?: string }) {
  ...
  if (workbench) {
    const handleAction = (item: WorkbenchItem) => {
      if (item.primary_action.type === 'open_candidate') return void openCandidate(Number(item.primary_action.id))
      if (item.primary_action.type === 'open_workflow') return void openWorkflow(item.primary_action.id)
      if (item.primary_action.type === 'open_analysis') return void openAnalysis?.(item.primary_action.id)
    }
    return <>
      <TodayWorkbench dashboard={dashboard} workbench={workbench} templates={templates} onAction={handleAction} onQuickAnalysis={() => quickAnalysis?.()} onRunTemplate={(id) => runTemplate?.(id)} onOpenTemplate={template => openTemplate?.(template)} onCreateTemplate={() => createTemplate?.()} onManageTemplate={template => manageTemplate?.(template)} busy={analysisBusy} />
