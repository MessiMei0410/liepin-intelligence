...
export function WorkflowPanel({value,jobs,close,reload,openCandidate,archived}:{value:Workflow,jobs:Job[],close:()=>void,reload:()=>void|Promise<void>,openCandidate:(id:number)=>void,archived:()=>void}) {
  ...
  const discussStrategy=async()=>{
    setBusy('copilot')
    try{
      await openCopilotWindow({type:'workflow',id:value.workflow.workflow_id,mode:'strategy_revision',client:target.client,job:target.job})
    }finally{setBusy('')}
  }
  ...
  const headline=['target_met','needs_review','pool_insufficient'].includes(mapped.kind) ? mapped.label : status==='waiting_approval' ? `已完成 ${completed}/${total} 步，等待外部寻访授权` : status==='completed' ? `工作流已完成，共 ${total} 步` : status==='failed' ? humanizeWorkflowError(failedStep?.error||value.goal.error) : status==='blocked' ? '工作流需要处理后继续' : status==='planned' ? '计划已就绪，等待确认' : current ? `正在处理：${current.business_label}` : workflowStatusLabel[status] || status
  return <div className="overlay"><article className="workflow-panel"><header className="detail-head"><button className="icon-btn" onClick={close} title="返回" aria-label="返回"><ChevronLeft/></button><div><h2>{value.goal.title}</h2><p>{value.workflow.workflow_id} · {mapped.label}</p></div><div className="detail-actions">{actionError&&<span className="tag warn">{actionError}</span>}<button className="button" disabled={!!busy} onClick={()=>void discussStrategy()}>{busy==='copilot'?<LoaderCircle className="spin"/>:<MessageSquareText/>}在 Copilot 中讨论策略</button>{archiveAllowed&&<button className="button" disabled={!!busy} onClick={()=>action('archive')}>{busy==='archive'?<LoaderCircle className="spin"/>:<Archive/>}归档</button>}{!['cancelled','completed'].includes(status)&&<button className="button" disabled={!!busy} onClick={()=>action('cancel')}>{busy==='cancel'&&<LoaderCircle className="spin"/>}取消</button>}{status==='planned'&&<button className="button primary" disabled={!!busy} onClick={()=>action('start')}>{busy==='start'?<LoaderCircle className="spin"/>:<Activity/>}确认计划并准备</button>}</div></header><div className="workflow-body"><main>
    ...
    {mapped.showNextActions&&<div className="workflow-next-actions" role="group" aria-label="下一步操作"><button className="button" disabled={!!busy} onClick={reviewCandidates}><UserRoundSearch/>复核现有人选</button><button className="button" disabled={!!busy} onClick={()=>void discussStrategy()}>{busy==='copilot'&&<LoaderCircle className="spin"/>}在 Copilot 中调整策略</button>{archiveAllowed&&<button className="button" disabled={!!busy} onClick={()=>action('archive')}>{busy==='archive'?<LoaderCircle className="spin"/>:<Archive/>}结束本轮</button>}</div>}
    <WorkflowTarget target={target} objective={value.goal.objective}/>
    <WorkflowStrategy strategy={strategy} channels={strategyChannels} gates={reviewGates} coverage={strategyCoverage} highlights={strategyHighlights} open={strategyOpen} toggle={()=>setStrategyOpen(value=>!value)}/>
