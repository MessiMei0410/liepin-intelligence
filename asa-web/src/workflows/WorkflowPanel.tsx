...
import { Activity, Archive, Ban, Check, ChevronLeft, CircleCheck, CircleDashed, Clock3, ExternalLink, FileText, ListChecks, LoaderCircle, MessageSquareText, Pause, Play, ShieldCheck, TriangleAlert, UserRoundSearch, X } from 'lucide-react'
...
export function WorkflowPanel({value:sourceValue,jobs,close,reload,openCandidate,archived,closeAll}:{value:Workflow,jobs:Job[],close:()=>void,reload:()=>void|Promise<void>,openCandidate:(id:number)=>void,archived:()=>void,closeAll?:()=>void}) {
  const {value,busy,error:actionError,feedback:actionFeedback,runAction:action,requestAction,pendingAction,cancelPendingAction,confirmPendingAction,decide,retry}=useWorkflowWriteActions({sourceValue,reload,archived})
  ...
  const discussStrategy=()=>openAgentWorkspace({type:'workflow',id:value.workflow.workflow_id,mode:'strategy_revision',client:target.client,job:target.job})
  ...
  return <DialogPanel panelClassName="workflow-panel" onEscape={close} onDetach={detachPanel} minWidth={320} minHeight={300}><header className="detail-head" style={{ cursor: 'grab', userSelect: 'none', touchAction: 'none' }} title="按住拖动；拖出屏幕边缘可弹出为独立窗口"><button className="icon-btn" onClick={close} title="返回" aria-label="返回"><ChevronLeft/></button><div><h2>{value.goal.title}</h2><p>{value.workflow.workflow_id} · {mapped.label}</p></div><div className="detail-actions">{actionError&&<span className="tag warn">{actionError}</span>}<button className="button" disabled={!!busy} onClick={()=>void discussStrategy()}>{busy==='copilot'?<LoaderCircle className="spin"/>:<MessageSquareText/>}在 Agent 中讨论策略</button>{archiveAllowed&&<button className="button" disabled={!!busy} onClick={()=>requestAction('archive')}>{busy==='archive'?<LoaderCircle className="spin"/>:<Archive/>}归档</button>}{live&&<button className="button" disabled={!!busy} onClick={()=>requestAction('pause')}>{busy==='pause'?<LoaderCircle className="spin"/>:<Pause/>}暂停寻访</button>}{status==='paused'&&<button className="button primary" disabled={!!busy} onClick={()=>requestAction('resume')}>{busy==='resume'?<LoaderCircle className="spin"/>:<Play/>}继续寻访</button>}{!['cancelled','completed'].includes(status)&&<button className="button danger" disabled={!!busy} onClick={()=>requestAction('cancel')}>{busy==='cancel'?<LoaderCircle className="spin"/>:<Ban/>}立即停止寻访</button>}{status==='planned'&&<button className="button primary" disabled={!!busy} onClick={()=>action('start')}>{busy==='start'?<LoaderCircle className="spin"/>:<Activity/>}确认计划并准备</button>}<button className="icon-btn candidate-dialog-detach" onClick={()=>void detachPanel()} title="弹出为独立窗口（可拖出屏幕）" aria-label="弹出为独立窗口"><ExternalLink/></button>{closeAll&&<button className="icon-btn" onClick={closeAll} title="关闭" aria-label="关闭"><X/></button>}</div></header>{actionFeedback&&<div className="candidate-action-feedback success" role="status"><CircleCheck/><span>{actionFeedback}</span></div>}<div className="workflow-body"><main>
    ...
    <StrategyReview workflowId={value.workflow.workflow_id} status={status} updatedAt={value.workflow.updated_at||''} openCandidate={openCandidate} jobId={contextJobId||undefined} mappingArtifactId={mappingArtifactId||undefined} onOpenMapping={openMappingCard} onChanged={reload}/>
    {mappingCardId&&contextJobId>0&&<MappingTaskCard jobId={contextJobId} artifactId={mappingCardId} openCandidate={openCandidate} onClose={()=>setMappingCardId('')} onChanged={reload}/>}
      ...
      <div className="sourcing-result-dialog">
        <SourcingResultCard
          ...
          onAction={(actionType)=>{
            ...
            else if(actionType==='archive')requestAction('archive')
    ...
    <div className="workflow-detail-events"><SectionHead title="执行动态" meta={`${events.length} 条`} />{events.length===0?<div className="aside-empty"><CircleDashed/><span>启动后将在这里显示过程</span></div>:<div className="workflow-events">{events.slice(0,12).map(e=><div key={e.id}><i className={stepTone(e.status)}/><span>{date(e.created_at)}</span><b>{humanizeWorkflowEvent(e)}</b></div>)}</div>}</div>
    <div className="workflow-detail-artifacts"><SectionHead title="结果与产物" meta={`${value.artifacts.length+(hasAssessmentResult?1:0)} 项`} />{hasAssessmentResult&&<div className="aside-item"><b>候选人核验结果</b><small>{assessmentResultSummary||`本轮评估 ${Number(assessmentQueue.started||0)} 位，岗位已评估 ${Number(assessmentQueue.completed||0)} 位`}</small></div>}{value.artifacts.length===0&&!hasAssessmentResult&&<div className="aside-empty artifact-empty"><CircleDashed/><span>{artifactAbsenceMessage(value)}</span></div>}{value.artifacts.map(a=><button type="button" className="aside-item artifact-item" key={a.artifact_id} onClick={()=>setArtifactCardId(a.artifact_id)} aria-label={`查看产物：${a.title}`}><FileText/><span><b>{a.title}</b><small>{artifactTypeLabel(a.artifact_type)} · {artifactStatusLabel(a.validation_status)}</small></span></button>)}</div></aside></div>{artifactCardId&&<WorkflowArtifactDialog key={artifactCardId} artifactId={artifactCardId} onClose={()=>setArtifactCardId('')}/>}{pendingAction&&<WorkflowActionConfirmDialog action={pendingAction} workflowTitle={value.goal.title} busy={!!busy} error={actionError} onConfirm={note=>void confirmPendingAction(note)} onCancel={cancelPendingAction}/>}</DialogPanel>
}
