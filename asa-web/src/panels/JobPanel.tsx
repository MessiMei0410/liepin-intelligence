...
export function JobPanel({value,close,openCandidate}:{value:JobDetail,close:()=>void,openCandidate:(id:number)=>void}) {
  ...
  return <div className="overlay"><article className="detail-panel job-detail-panel"><header className="detail-head"><button className="icon-btn" onClick={close} title="返回" aria-label="返回"><ChevronLeft/></button><div><h2>{value.title}</h2><p>{value.client} · {position.department||position.team||value.location||'岗位详情'} · 岗位 #{value.id}</p></div><div className="detail-actions"><button className="button" onClick={()=>openAgentWorkspace({type:'job',id:value.id,client:value.client,job:value.title})}><MessageSquareText/>交给 Agent</button>{value.priority?.includes('P0')&&<span className="tag warn">P0 最急</span>}<button className="icon-btn" onClick={close} title="关闭" aria-label="关闭"><X/></button></div></header><div className="job-detail-body"><main>
    ...
    <section className="job-detail-section"><h3>当前寻访策略</h3>{strategy?<><p className="job-summary">{strategy.summary||'策略已生成，等待执行验证。'} · 计划 v{strategy.plan_version}</p>{strategy.consultant_constraints.length>0&&<div className="job-list-items">{strategy.consultant_constraints.map(item=><div key={`${item.type}:${item.rule}`}><Check/><span>{item.rule}</span></div>)}</div>}{strategyKeywords.length>0&&<div className="job-keywords">{strategyKeywords.map(item=><span key={item}>{item}</span>)}</div>}{strategyCompanies.length>0&&<p className="job-summary">目标公司：{strategyCompanies.join('、')}</p>}{strategyLevels.length>0&&<p className="job-summary">可接受职级：{strategyLevels.join('、')}</p>}{strategyFallback&&<p className="job-summary">扩展路径：{strategyFallback}</p>}</>:<div className="empty">当前岗位还没有生效的寻访策略。</div>}{risks.length>0&&<div className="job-risk-list">{risks.map(item=><p key={item}><ShieldAlert/>{item}</p>)}</div>}</section>
    <section className="job-detail-section"><h3>寻访记录</h3>{value.search_experiments.length>0?<div className="job-experiments">{value.search_experiments.slice(0,12).map(item=><div key={item.id}><Route/><div><b>{String(item.query||'未记录关键词')}</b><span>{sourceLabel(String(item.channel||''))} · 结果 {Number(item.result_count||0)} · 提取 {Number(item.extracted_count||0)} · 推荐 {Number(item.recommended_count||0)}</span></div><small>{date(String(item.updated_at||item.run_time||''))}</small></div>)}</div>:<div className="empty">暂无历史寻访记录。</div>}</section>
