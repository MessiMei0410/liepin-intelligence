...
import { api } from '../api'
import type { SourcingAdjustmentEffect, SourcingAdjustmentListPayload, SourcingAdjustmentStatus, SourcingAdjustmentType } from '../api'
import { copilotText } from '../shared/text'
import { date } from '../shared/format'

// 停止备注 → 寻访调整：停止候选人时填写的备注经 LLM 分析成下一轮寻访调整指令。
// 待应用列表人工确认/忽略（确认等同已应用），已处理折叠展示。
...
export function SourcingAdjustments({ jobId }: { jobId: number }) {
  ...
  const act = async (id: number, kind: 'confirm' | 'ignore') => {
    ...
    setActionError('')
    try {
      if (kind === 'confirm') await api.confirmSourcingAdjustment(id)
      else await api.ignoreSourcingAdjustment(id)
      await load()
    } catch (reason) {
      setActionError(copilotText(reason) || (kind === 'confirm' ? '确认失败，请重试' : '忽略失败，请重试'))
    } finally {
      setBusyId(null)
    }
  }
  ...
  return (
    <section className="job-detail-section sourcing-adjustments" aria-label="待应用调整">
      <h3>
        待应用调整{pending.length > 0 && <span className="job-section-count">{pending.length} 条待处理</span>}
      </h3>
      ...
      {pending.length > 0 && (
        <div className="sourcing-adjust-list" aria-label="待应用调整列表">
          {pending.map(item => (
            <div className="sourcing-adjust-item" key={item.id}>
              ...
              <div className="sourcing-adjust-actions">
                <button type="button" className="button confirm" disabled={busyId === item.id} onClick={() => void act(item.id, 'confirm')}>
                  {busyId === item.id ? <LoaderCircle className="spin" /> : <Check />}确认应用
                </button>
      ...
      {history.length > 0 && (
        <div className="sourcing-adjust-history">
          <button type="button" className="button sourcing-adjust-toggle" aria-expanded={showHistory} onClick={() => setShowHistory(shown => !shown)}>
            <ChevronDown className={showHistory ? 'rotate' : ''} />已处理 {history.length} 条（已应用 {appliedCount} / 已忽略 {history.length - appliedCount}）
          </button>
          {showHistory && (
