...
import { Ban, BriefcaseBusiness, CalendarPlus, Check, ChevronLeft, ChevronRight, CircleCheck, ClipboardCheck, Clock3, ExternalLink, FileText, LoaderCircle, MessageSquareText, ShieldCheck, TriangleAlert, UserRoundSearch, X } from 'lucide-react'
...
import { extractResumeSection, filterResumeChromeLines, parseEducationDetails, parseProjectDetails, parseWorkDetails } from './resumeDetail'
...
// 详情页"上一位/下一位"导航：prevId/nextId 缺省表示已到名单端点，index/total 用于展示当前位次。
export type CandidateNav = { prevId?: number; nextId?: number; index: number; total: number }
...
export function CandidatePanel({value,close,changed,nav,onNavigate}:{value:CandidateDetail,close:()=>void,changed:()=>void|Promise<void>,nav?:CandidateNav,onNavigate?:(id:number)=>void}) {
  ...
  const refreshAfterWrite=async()=>{try{await changed()}catch{/* 写入已成功，详情回读失败由后续轮询收敛。 */}}
  ...
  const commitAction=async()=>{
    ...
    try {
      ...
      if(action==='recommend'){
        ...
        } finally { void refreshAfterWrite() }
      } else {
        setPendingAction(undefined)
      ...
      const message=copilotText(e)||'操作提交失败，请重试。'
      setFeedback({tone:'error',text:`${message} 正在重新读取候选人状态。`})
      await Promise.resolve(changed()).catch(()=>undefined)
      setFeedback({tone:'error',text:`${message} 已重新读取候选人状态。`})
    } finally {
      setBusy('')
    }
  }
  ...
  return <><DialogPanel panelClassName="detail-panel candidate-panel" onEscape={close} onDetach={detachPanel}><header className="detail-head candidate-head" style={{ cursor: 'grab', userSelect: 'none', touchAction: 'none' }} title="按住拖动；拖出屏幕边缘可弹出为独立窗口"><div className="candidate-head-primary"><button className="icon-btn" onClick={close} title="返回" aria-label="返回"><ChevronLeft/></button>{nav&&onNavigate&&<span className="candidate-nav" role="group" aria-label="切换候选人"><button className="icon-btn" disabled={nav.prevId===undefined} onClick={()=>{if(nav.prevId!==undefined)onNavigate(nav.prevId)}} title="上一位" aria-label="上一位人选"><ChevronLeft/></button><span className="candidate-nav-index" aria-label={`第 ${nav.index+1} 位，共 ${nav.total} 位`}>{nav.index+1}/{nav.total}</span><button className="icon-btn" disabled={nav.nextId===undefined} onClick={()=>{if(nav.nextId!==undefined)onNavigate(nav.nextId)}} title="下一位" aria-label="下一位人选"><ChevronRight/></button></span>}<div><h2>{value.name}</h2><p>{value.current_title || '职位待补充'} · {value.current_company || '公司待补充'} · {value.city || '城市待补充'}</p></div></div><div className="detail-actions"><button className="button" onClick={()=>openAgentWorkspace({type:'candidate',id:value.id,candidate:value.name,client:value.client,job:value.job})}><MessageSquareText/>交给 Agent</button>{links.map(link=><a className="button" href={link.source_url} target="_blank" rel="noreferrer" title={`打开${sourceLinkLabel(link.source_system)}`} key={sourceLinkLabel(link.source_system)}>{sourceLinkLabel(link.source_system)}<ExternalLink/></a>)}{value.is_stopped?<span className="button danger"><Ban/>已停止推进{value.stop_reason_label?` · ${value.stop_reason_label}`:''}</span>:<><button className="button" disabled={!!busy} onClick={()=>act('review')}>{busy==='preflight:review'?<LoaderCircle className="spin"/>:<ClipboardCheck/>}评分复核</button><button className="button" disabled={!!busy||reviewPassed} onClick={()=>act('advance')}>{busy==='preflight:advance'?<LoaderCircle className="spin"/>:reviewPassed?<Check/>:null}复核通过</button><button className="button" disabled={!!busy||contacted} onClick={()=>act('contact')}>{busy==='preflight:contact'?<LoaderCircle className="spin"/>:contacted?<Check/>:null}已联系</button><button className="button primary" disabled={!!busy||recommended} onClick={()=>act('recommend')}>{busy==='preflight:recommend'?<LoaderCircle className="spin"/>:recommended?<Check/>:null}已推荐</button><button className="button danger" disabled={!!busy} onClick={()=>act('stop')}>{busy==='preflight:stop'?<LoaderCircle className="spin"/>:<Ban/>}停止</button></>}<button className="icon-btn candidate-dialog-detach" onClick={()=>void detachPanel()} title="弹出为独立窗口（可拖出屏幕）" aria-label="弹出为独立窗口"><ExternalLink/></button><button className="icon-btn" onClick={close} title="关闭" aria-label="关闭"><X/></button></div></header>
      ...
      <div className="detail-body"><main><div className="candidate-main-content">
        ...
        {view==='assessment'&&<CandidateAssessment candidateId={value.id} jobId={value.job_id} onChanged={changed}/>}
        {view==='resume'&&<div className="resume-workspace"><ResumeWorkDetail text={resume.full_text} fallback={<ResumeTimelineSection title="工作经历" text={resume.work_text} empty="尚未采集结构化工作经历，可通过来源链接核对原始简历。"/>}/><ResumeProjectSection text={resume.project_text} fullText={resume.full_text}/><ResumeEducationSection text={resume.education_text} fullText={resume.full_text}/>{resume.full_text&&<details className="raw-resume"><summary>完整原始履历</summary><pre>{filterResumeChromeLines(resume.full_text)}</pre></details>}</div>}
...
function ResumeProjectSection({text,fullText}:{text?:string;fullText?:string}) {
  const source=String(text||'').trim()||extractResumeSection(fullText||'','project')
  const projects=parseProjectDetails(source)
...
function ResumeEducationSection({text,fullText}:{text?:string;fullText?:string}) {
  const source=String(text||'').trim()||extractResumeSection(fullText||'','education')
  const education=parseEducationDetails(source)
