...
export function CandidatePanel({value,close,changed}:{value:CandidateDetail,close:()=>void,changed:()=>void|Promise<void>}) {
  ...
  const links=[...(value.source_links||[]).reduce((selected,link)=>{
    const label=sourceLinkLabel(link.source_system)
    if(selected.has(label)||!isWebSourceUrl(link.source_url))return selected
    selected.set(label,link)
    return selected
  },new Map<string,CandidateDetail['source_links'][number]>()).values()]
...
function isWebSourceUrl(value?:string):value is string {
