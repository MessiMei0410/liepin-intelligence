...
const reviewPayload = {
  ...
  review: {
    ...
    verdict_reason: '本轮总召回 120；轮次信号 pool_saturated：排重率 99%（119/120）> 阈值 80%，本地池已枯竭，已生成扩池决策树（信号与判定正交）',
    degraded: false,
    signals: [
      {
        ...
        detail: '本轮抽取 120 条、排重 119 条，轮次排重率 99.2% > 阈值 80%：本地候选池已枯竭，须换打法而非原样重搜',
    ...
    expansion_decision_tree: [
      ...
      },
      {
        ...
        params: {
          items: [
            { field: '年限', current: null, proposal: '', cost: '', source: 'none', note: 'strategy_v2 未记录年限门槛，当前值取不到；放宽幅度由顾问定' },
...
describe('复盘卡扩区：池枯竭信号 + 扩池决策树（StrategyExpansionTree）', () => {
  it('渲染信号 tag 与 detail；决策树按 order 渲染 5 步全字段与 params 摘要，缺省项如实"待顾问补充"', async () => {
    ...
    expect(within(section).getByText('池枯竭（排重率过高）')).toBeInTheDocument()
    expect(within(section).getByText(/本轮抽取 120 条、排重 119 条，轮次排重率 99.2% > 阈值 80%/)).toBeInTheDocument()
    // 决策树头与 5 步编号 + action_type 中文名 + title
    expect(within(section).getByText('扩池决策树')).toBeInTheDocument()
    ...
    expect(within(section).getByText('5. 转 Mapping 校准')).toBeInTheDocument()
    ...
    expect(within(section).getByText('年限：待顾问补充 → 待顾问补充；strategy_v2 未记录年限门槛，当前值取不到；放宽幅度由顾问定')).toBeInTheDocument()
    ...
    expect(within(section).getByText('升级路径：Mapping 直挖、与客户校准方向')).toBeInTheDocument()
    ...
    expect(steps[4].textContent).toContain('5. 转 Mapping 校准')
  })
  ...
  it('树乱序到达时仍按 order 升序渲染', async () => {
    ...
    const { container } = render(<StrategyReview workflowId="wf-1" status="blocked" updatedAt="" />)
    await screen.findByRole('region', { name: '策略复盘' })
    const steps = container.querySelectorAll('.review-tree-step')
    expect(steps).toHaveLength(5)
    expect(steps[0].textContent).toContain('1. 换关键词组')
  ...
  it('无信号无树的旧复盘不渲染新区块', async () => {
    ...
    expect(within(section).queryByText('扩池决策树')).not.toBeInTheDocument()
    expect(within(section).queryByText('池枯竭（排重率过高）')).not.toBeInTheDocument()
...
describe('修改计划对话框接扩池决策树（RevisePlanDialog）', () => {
  const openDialog = (onSubmit: (instruction: string) => void) => {
    ...
    return { fetchMock, region: screen.findByLabelText('扩池决策树') }
  }
  ...
  it('对话框内渲染决策树 5 步（编号 + 中文名 + title + params 摘要），每步带采纳/拒绝按钮；diff 区块并存', async () => {
    ...
    expect(within(tree).getByText('5. 转 Mapping 校准')).toBeInTheDocument()
    ...
    // 既有 diff 区块并存不回归
    expect(screen.getByLabelText('策略复盘修订建议')).toBeInTheDocument()
  })
  ...
  it('提交时树决策与 diff 决策清单一并并入 instruction 尾部', async () => {
    ...
    const tree = await region
    const diffRegion = screen.getByLabelText('策略复盘修订建议')
  ...
  it('无树的旧复盘不渲染树区块，diff 采纳与提交后缀不回归', async () => {
    ...
    stubReviewFetch(legacyPayload)
    render(<RevisePlanDialog workflowId="wf-1" onCancel={() => undefined} onSubmit={onSubmit} />)
    const diffRegion = await screen.findByLabelText('策略复盘修订建议')
    expect(screen.queryByLabelText('扩池决策树')).not.toBeInTheDocument()
    await user.click(within(diffRegion).getByRole('button', { name: '采纳' }))
    const textarea = screen.getByRole('textbox')
...
describe('工作流面板：调整条件再搜 → 树步骤采纳 → revise 提交链路', () => {
  it('采纳树步骤预填后提交，revise 请求体带预填内容与采纳步骤后缀', async () => {
    ...
    const tree = await within(dialog).findByLabelText('扩池决策树')
    await user.click(within(tree).getAllByRole('button', { name: '采纳' })[1])
