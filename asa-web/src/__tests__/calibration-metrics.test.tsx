...
describe("评估校准 · 顾问点头率（CalibrationMetrics）", () => {
  it("渲染维度×客户表格，率值按百分比呈现", async () => {
    ...
    expect(
      screen.getByText("已评估 5 份 · 已采纳 3 · 已改判 1 · 已否决 0"),
    ).toBeTruthy();
    expect(screen.getByRole("columnheader", { name: "客户" })).toHaveAttribute(
      "scope",
      "col",
    );
    expect(
      screen.getByRole("columnheader", { name: "顾问点头率" }),
  ...
  it("空数据与加载失败都有降级文案", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/还没有足够的顾问动作数据/)).toBeTruthy(),
    );
    unmount();
    stubMetrics({ error: "boom" }, false, 500);
    render(<CalibrationMetrics />);
    await waitFor(() =>
      expect(screen.getByText(/校准数据暂时加载失败/)).toBeTruthy(),
    );
  });

  it("加载失败后可重新加载，重试成功恢复表格", async () => {
    const fetchMock = vi
      .fn<typeof fetch>()
      .mockResolvedValueOnce(mockResponse({ error: "boom" }, false, 500))
      .mockResolvedValueOnce(mockResponse(metricsPayload));
    vi.stubGlobal("fetch", fetchMock);
    render(<CalibrationMetrics />);

    const retryButton = await screen.findByRole("button", { name: "重新加载" });
    expect(screen.getByRole("alert")).toHaveTextContent(/校准数据暂时加载失败/);
    fireEvent.click(retryButton);

    expect(screen.getByText(/校准数据加载中/)).toBeInTheDocument();
