...
const DRAFT_KEY = 'revise-plan-draft'
...
export function RevisePlanDialog({ workflowId, onCancel, onSubmit }: { workflowId?: string; onCancel: () => void; onSubmit: (instruction: string) => void }) {
  const [instruction, setInstruction] = useState(() => sessionStorage.getItem(DRAFT_KEY) ?? '')
  ...
  useEffect(() => {
    sessionStorage.setItem(DRAFT_KEY, instruction)
  }, [instruction])
